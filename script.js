...
document.addEventListener('DOMContentLoaded', function() {
      ...
      (async function() {
        try {
          ...
          const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: '983155f7-2211-4fe4-8534-1d0a03b12b80',
              name: data.name || '',
              email: data.email || '',
  ...
  if (reservationForm) {
    reservationForm.addEventListener('submit', function(e) {
      ...
      (async function() {
        try {
          console.log('📧 Zappy: Sending reservation form to backend...');
          const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              ...
              name: data.name || '',
              email: data.email || '',
              subject: data.subject || 'Reservation Request',
              message: data.message || (data.date ? 'Reservation for: ' + data.date + (data.time ? ' at ' + data.time : '') + (data.guests ? ' for ' + data.guests + ' guests' : '') : ''),
              phone: data.phone || null
            })
          });
          const result = await response.json();
          console.log('✅ Zappy: Reservation email sent successfully', result);
        } catch (error) {
          console.error('❌ Zappy: Failed to send reservation email', error);
          // Don't break existing functionality
        }
      })();
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Claudemenu משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
